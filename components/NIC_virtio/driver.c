...
static inline buffer_t get_sub_buffer_with_min_len(buffer_t *buffer, size_t offset,
                                            size_t min_len)
{
...
OS_Error_t nic_rpc_tx_data(size_t* pLen) {
	...
	*pLen = 0;
    ...
    int err = camkes_virtqueue_driver_scatter_send_buffer(vq, ctx->nw_buffer_from.buffer, len);
    if (err) {
...
OS_Error_t nic_rpc_rx_data(size_t* pLen, size_t* framesRemaining) {
	...
	OS_NetworkStack_RxBuffer_t* nw_rx = (OS_NetworkStack_RxBuffer_t*) ctx->nw_buffer_to.buffer;
    
    size_t len = virtqueue_scattered_available_size(vq, &handle);
    if (camkes_virtqueue_device_gather_copy_buffer(vq, &handle, nw_rx->data, len) < 0) {
